...
import { useSelector } from 'react-redux';
 ...
 function dateAdapter(dateFr) {
   if (dateFr) {
     const [d, m, y] = dateFr.split('/');
     const valideDate = new Date(y, m - 1, d);
 ...
 const columns = [
   ...
   },
   {
     ...
     type: 'date',
     valueGetter: (date) => dateAdapter(date),
     width: 100,
   },
 ...
 export default function DataTable() {
   ...
   const rowsConstructor = useCallback(
     (list) =>
       list.map((el, index) => {
         return { ...el, id: `${index}-${el.firstName}-${el.lastName}-(${el.dateOfBirth})` };
       }),
     []
   );
